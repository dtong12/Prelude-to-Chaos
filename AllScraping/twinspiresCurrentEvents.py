...
headers = {
  ...
  'Origin': 'https://az.twinspires.com',
  'Referer': 'https://az.twinspires.com/',
...
def get_tennis_events():
    url = "https://eu-offering.kambicdn.org/offering/v2018/winusaz/event/live/open.json?lang=en_US&market=US-AZ&client_id=2&ncid=1669769628201"
...
async def async_get_event_data(session, event_id):
    url = f"https://eu-offering.kambicdn.org/offering/v2018/winusaz/betoffer/event/{event_id}?type=&includeParticipants=true&lang=en_US&market=US"
    print(url)
...
def async_twinspires_main():
    ...
    asyncio.run(main())
    with open('twinspires_current_event.json', 'w') as f:
        json.dump(ans, f)
    return {'timestamp': timestamp, 'ans': ans}

if __name__ == '__main__':
    async_twinspires_main()
